-- ╔═══════════════════════════════════════════════════════════════════════╗
-- ║  AWC Group — Division Template Seeds                                  ║
-- ║                                                                       ║
-- ║  `seed_division_templates(business_id, division)` inserts the default ║
-- ║  job types and products library for a given division.                 ║
-- ║  Called from the app during Onboarding and when enabling a division.  ║
...
AS $$
BEGIN
  -- Guard: caller must own the business (or be a staff member of it)
  IF _business_id IS NULL OR _business_id <> current_business_id() THEN
...
GRANT EXECUTE ON FUNCTION seed_division_templates(UUID, TEXT) TO authenticated;
